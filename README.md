...
- **[Последовательности](#последовательности)**
  ...
  - [Какие операции поддерживают большинство последовательностей](#какие-операции-поддерживают-большинство-последовательностей)
  - [Какие виды строк бывают в питоне](#какие-виды-строк-бывают-в-питоне)
  - [Можно ли изменить отдельный символ внутри строки](#можно-ли-изменить-отдельный-символ-внутри-строки)
  - [Как соединить список строк в одну. Как разбить строку на список строк](#как-соединить-список-строк-в-одну-как-разбить-строку-на-список-строк)
  - [Как кодировать и декодировать строки](#как-кодировать-и-декодировать-строки)
  - [Чем список отличается от кортежа](#чем-список-отличается-от-кортежа)
  - [Что такое диапазон](#что-такое-диапазон)
  - [Как сделать список уникальным (без повторяющихся элементов)](#как-сделать-список-уникальным-без-повторяющихся-элементов)
  - [Есть кортеж из трех элементов. Назначить переменным a, b, c его значения](#есть-кортеж-из-трех-элементов-назначить-переменным-a-b-c-его-значения)
  - [Как сравниваются последовательности](#как-сравниваются-последовательности)
- **[Множества и отображения](#множества-и-отображения)**
  - [Как понять хешируемый ли объект](#как-понять-хешируемый-ли-объект)
  - [Что такое множество](#что-такое-множество)
  - [Для чего применяются множества](#для-чего-применяются-множества)
  - [Какие операции можно производить над множествами](#какие-операции-можно-производить-над-множествами)
  - [Как происходит проверка множеств на равенство](#как-происходит-проверка-множеств-на-равенство)
  - [Что такое отображение](#что-такое-отображение)
  - [Какие нюансы есть в использовании чисел как ключей](#какие-нюансы-есть-в-использовании-чисел-как-ключей)
  - [Какие операции можно производить над отображениями](#какие-операции-можно-производить-над-отображениями)
  - [Что возвращает метод items](#что-возвращает-метод-items)
  - [Как отсортировать список словарей по определенному полю](#как-отсортировать-список-словарей-по-определенному-полю)
  - [Что может являться ключом словаря. Что не может. Почему](#что-может-являться-ключом-словаря-что-не-может-почему)
  - [Есть два списка – ключи и значения. Как составить из них словарь](#есть-два-списка-–-ключи-и-значения-как-составить-из-них-словарь)
  - [Как работает хэш-таблица](#как-работает-хэш-таблица)
  - [Что такое коллизия](#что-такое-коллизия)
  - [Где будет быстрее поиск, а где перебор и почему: dict, list, set, tuple](#где-будет-быстрее-поиск-а-где-перебор-и-почему-dict-list-set-tuple)
- **[Функции](#функции)**
  - [Что такое args, kwargs. В каких случаях они требуются](#что-такое-args-kwargs-в-каких-случаях-они-требуются)
  - [Почему использовать изменяемые объекты как параметры по-умолчанию плохо. Приведите пример плохого случая. Как исправить](#почему-использовать-изменяемые-объекты-как-параметры-по-умолчанию-плохо-приведите-пример-плохого-случая-как-исправить)
  - [Можно ли передавать функцию в качестве аргумента другой функции](#можно-ли-передавать-функцию-в-качестве-аргумента-другой-функции)
  - [Можно ли объявлять функцию внутри другой функции. Где она будет видна](#можно-ли-объявлять-функцию-внутри-другой-функции-где-она-будет-видна)
  - [Что такое лямбды. Каковы их особенности](#что-такое-лямбды-каковы-их-особенности)
  - [Допустимы ли следующие выражения](#допустимы-ли-следующие-выражения)
  - [Как передаются значения аргументов в функцию или метод](#как-передаются-значения-аргументов-в-функцию-или-метод)
...
- **[Итераторы и генераторы](#итераторы-и-генераторы)**
  ...
  - [Что такое итерабельный объект](#что-такое-итерабельный-объект)
  - [Что такое итератор](#что-такое-итератор)
  - [Что такое генератор](#что-такое-генератор)
  - [Что такое генераторная функция](#что-такое-генераторная-функция)
  - [Что делает yield](#что-делает-yield)
  - [В чем отличие \[x for x in y\] от (x for x in y)](#в-чем-отличие-x-for-x-in-y-от-x-for-x-in-y)
  - [Что особенного в генераторе](#что-особенного-в-генераторе)
  - [Как объявить генератор](#как-объявить-генератор)
  - [Как получить из генератора список](#как-получить-из-генератора-список)
  - [Что такое подгенератор](#что-такое-подгенератор)
  - [Какие методы есть у генераторов](#какие-методы-есть-у-генераторов)
  - [Можно ли извлечь элемент генератора по индексу](#можно-ли-извлечь-элемент-генератора-по-индексу)
  - [Что возвращает итерация по словарю](#что-возвращает-итерация-по-словарю)
  - [Как итерировать словарь по парам ключ-значение](#как-итерировать-словарь-по-парам-ключ-значение)
  - [Что такое сопрограмма](#что-такое-сопрограмма)
- **[Классы, объекты](#классы-объекты)**
  - [Как получить список атрибутов объекта](#как-получить-список-атрибутов-объекта)
  - [Что такое магические методы, для чего нужны](#что-такое-магические-методы-для-чего-нужны)
  - [Как в классе сослаться на родительский класс](#как-в-классе-сослаться-на-родительский-класс)
  - [Возможно ли множественное наследование](#возможно-ли-множественное-наследование)
  - [Что такое MRO](#что-такое-mro)
  - [Что такое Diamond problem](#что-такое-diamond-problem)
  - [Что такое миксины](#что-такое-миксины)
  - [Что такое контекстный менеджер. Как написать свой](#что-такое-контекстный-менеджер-как-написать-свой)
  - [Прокомментировать выражение](#прокомментировать-выражение)
  - [Что такое \_\_slots\_\_. Плюсы, минусы](#что-такое-slots-плюсы-минусы)
  - [В чем смысл параметров _value, __value](#в-чем-смысл-параметров-value-value)
  - [Что такое \_\_new\_\_. И чем он отличается от \_\_init\_\_. В какой последовательности они выполняются](#что-такое-new-и-чем-он-отличается-от-init-в-какой-последовательности-они-выполняются)
  - [Что такое и чем отличается old-style от new-style classes](#что-такое-и-чем-отличается-old-style-от-new-style-classes)
  - [Что такое утиная типизация](#что-такое-утиная-типизация)
- **[Модули, пакеты](#модули-пакеты)**
  - [Что такое модуль](#что-такое-модуль)
  - [Как можно получить имя модуля](#как-можно-получить-имя-модуля)
  - [Что такое модульное программирование](#что-такое-модульное-программирование)
  - [Как Python ищет модули при импорте](#как-python-ищет-модули-при-импорте)
  - [Что такое пакет](#что-такое-пакет)
  - [Что вы можете сказать о конструкции import package.item](#что-вы-можете-сказать-о-конструкции-import-packageitem)
- **[Исключения](#исключения)**
  - [Что такое обработка исключений](#что-такое-обработка-исключений)
  - [Для чего могут применять конструкцию try finally без except](#для-чего-могут-применять-конструкцию-try-finally-без-except)
  - [Как правильно по-разному обрабатывать исключения](#как-правильно-по-разному-обрабатывать-исключения)
  - [Что будет если ошибку не обработает блок except](#что-будет-если-ошибку-не-обработает-блок-except)
  - [Что делать если нужно перехватить исключение, выполнить действия и опять возбудить это же исключение](#что-делать-если-нужно-перехватить-исключение-выполнить-действия-и-опять-возбудить-это-же-исключение)
  - [Что такое сцепление исключений](#что-такое-сцепление-исключений)
  - [Зачем нужен блок else](#зачем-нужен-блок-else)
  - [Что можно передать в конструктор исключения](#что-можно-передать-в-конструктор-исключения)
  - [Какие есть классы исключений](#какие-есть-классы-исключений)
  - [В каких случаях можно обработать SyntaxError](#в-каких-случаях-можно-обработать-syntaxerror)
  - [Можно ли создавать собственные исключения](#можно-ли-создавать-собственные-исключения)
  - [Для чего нужны предупреждения (warnings) и как создать собственное](#для-чего-нужны-предупреждения-warnings-и-как-создать-собственное)
  - [Для чего нужен модуль warnings](#для-чего-нужен-модуль-warnings)
- **[Декораторы](#декораторы)**
  - [Что такое декораторы. Зачем нужны](#что-такое-декораторы-зачем-нужны)
  - [Что может быть декоратором. К чему может быть применен декоратор](#что-может-быть-декоратором-к-чему-может-быть-применен-декоратор)
  - [Что будет, если декоратор не возвращает ничего](#что-будет-если-декоратор-не-возвращает-ничего)
  - [В чем отличие \@foobar от \@foobar()](#в-чем-отличие-foobar-от-foobar)
  - [Что такое фабрика декораторов](#что-такое-фабрика-декораторов)
  - [Зачем нужен wraps](#зачем-нужен-wraps)
- **[Метаклассы](#метаклассы)**
  - [Что такое метаклассы](#что-такое-метаклассы)
  - [Что такое type. Как работает поиск метакласса при создании объекта](#что-такое-type-как-работает-поиск-метакласса-при-создании-объекта)
  - [Как работают метаклассы](#как-работают-метаклассы)
  - [Зачем вообще использовать метаклассы](#зачем-вообще-использовать-метаклассы)
- **[Ввод-Вывод](#ввод-вывод)**
  - [Что такое файловый объект](#что-такое-файловый-объект)
  - [Какие есть виды файловых объектов](#какие-есть-виды-файловых-объектов)
  - [В чем отличие текстовых и бинарных файлов](#в-чем-отличие-текстовых-и-бинарных-файлов)
  - [Как пользоваться функцией open](#как-пользоваться-функцией-open)
  - [Для чего необходимо закрывать файлы](#для-чего-необходимо-закрывать-файлы)
  - [Что делают методы tell и seek](#что-делают-методы-tell-и-seek)
  - [Что делают StringIO и BytesIO](#что-делают-stringio-и-bytesio)
  - [Являются ли файловые объекты контекстными менеджерами](#являются-ли-файловые-объекты-контекстными-менеджерами)
  - [Что такое сериализация](#что-такое-сериализация)
  - [json.dumps / json.dump , json.loads / json.load](#jsondumps-jsondump-jsonloads-jsonload)
  - [Что делать если нужно сериализовать данные, которые не поддерживаются стандартным модулем json](#что-делать-если-нужно-сериализовать-данные-которые-не-поддерживаются-стандартным-модулем-json)
  - [pickle.dumps / pickle.dump, pickle.loads / pickle.load](#pickledumps-pickledump-pickleloads-pickleload)
- **[Тестирование](#тестирование)**
  - [Пирамида тестирования](#пирамида-тестирования)
  - [Что такое mocking](#что-такое-mocking)
  - [Что делать, если тестируемая функция использует удалённое подключение к внешним сервисам, которое иногда видает ошибку таймаута, 404 и им подобные](#что-делать-если-тестируемая-функция-использует-удалённое-подключение-к-внешним-сервисам-которое-иногда-видает-ошибку-таймаута-404-и-им-подобные)
  - [Что делать, если тестируемая функция занимает много времени на выполнение повторяющихся операций внутри неё](#что-делать-если-тестируемая-функция-занимает-много-времени-на-выполнение-повторяющихся-операций-внутри-неё)
  - [Какие вы знаете виды тестов](#какие-вы-знаете-виды-тестов)
  - [Чем интеграционное тестирование отличается от функционального](#чем-интеграционное-тестирование-отличается-от-функционального)
- **[Функциональное программирование](#функциональное-программирование)**
  ...
  - [Как у Python с поддержкой функционального программирования](#как-у-python-с-поддержкой-функционального-программирования)
  - [Что такое объект первого класса](#что-такое-объект-первого-класса)
  - [Что такое функция высшего порядка](#что-такое-функция-высшего-порядка)
  - [Что такое каррирование](#что-такое-каррирование)
  - [Опишите функции map, reduce, filter модуля functools](#опишите-функции-map-reduce-filter-модуля-functools)
  - [Какие еще вы знаете функции из модуля functools](#какие-еще-вы-знаете-функции-из-модуля-functools)
  - [Какие вы функции знаете из модуля itertools](#какие-вы-функции-знаете-из-модуля-itertools)
  - [Для чего нужен модуль operator](#для-чего-нужен-модуль-operator)
- **[GIL, потоки, процессы](#gil-потоки-процессы)**
  - [Что такое GIL. Какие у него есть проблемы](#что-такое-gil-какие-у-него-есть-проблемы)
  - [Работали ли Вы с asyncio. В чём его особенность](#работали-ли-вы-с-asyncio-в-чём-его-особенность)
  - [Что такое async/await, для чего они нужны и как их использовать](#что-такое-asyncawait-для-чего-они-нужны-и-как-их-использовать)
  - [Как в питоне реализуется многопоточность. Какими модулями](#как-в-питоне-реализуется-многопоточность-какими-модулями)
  - [В чем отличие тредов от мультипроцессинга](#в-чем-отличие-тредов-от-мультипроцессинга)
  - [Какие задачи хорошо параллелятся, какие плохо](#какие-задачи-хорошо-параллелятся-какие-плохо)
  - [Нужно посчитать 100 уравнений. Делать это в тредах или нет](#нужно-посчитать-100-уравнений-делать-это-в-тредах-или-нет)
  - [Треды в Питоне — это нативные треды или нет](#треды-в-питоне-—-это-нативные-треды-или-нет)
  - [Что такое гринлеты. Общее понятие. Примеры реализаций](#что-такое-гринлеты-общее-понятие-примеры-реализаций)

## [Прочее (Python)](#прочее-python)

- **[Какие варианты реализации шаблона Singleton на питоне](#какие-варианты-реализации-шаблона-singleton-на-питоне)**
- **[Какие вы знаете инструменты для проверки кодстайл](#какие-вы-знаете-инструменты-для-проверки-кодстайл)**
- **[Что такое list/dict comprehension](#что-такое-listdict-comprehension)**
- **[Какая разница между одинарным и двойным подчеркиванием](#какая-разница-между-одинарным-и-двойным-подчеркиванием)**
- **[Отличие copy() от deepcopy()](#отличие-copy-от-deepcopy)**
- **[Что такое garbage collector. В чём его плюсы и минусы](#что-такое-garbage-collector-в-чём-его-плюсы-и-минусы)**
- **[Что такое интроспекция](#что-такое-интроспекция)**
- **[Что такое рефлексия](#что-такое-рефлексия)**

## [Django](#django)

- **[Что такое Middleware, для чего, как реализуется](#что-такое-middleware-для-чего-как-реализуется)**
- **[Назовите основные мидлвари. Зачем они нужны](#назовите-основные-мидлвари-зачем-они-нужны)**
- **[Опишите алгоритм работы CSRF middleware](#опишите-алгоритм-работы-csrf-middleware)**
- **[Что такое сигналы? Зачем нужны? Назовите основные](#что-такое-сигналы-зачем-нужны-назовите-основные)**
- **[Как реализуется связь m2m на уровне базы данных](#как-реализуется-связь-m2m-на-уровне-базы-данных)**
- **[Чем лучше отправлять форму — GET или POST](#чем-лучше-отправлять-форму-—-get-или-post)**
- **[Как работает Serializer в Django REST Framework](#как-работает-serializer-в-django-rest-framework)**
- **[Что такое Meta в классах Django и для чего нужен](#что-такое-meta-в-классах-django-и-для-чего-нужен)**
- **[За что отвечает Meta в сериализаторе](#за-что-отвечает-meta-в-сериализаторе)**
- **[Какая разница в быстродействии между django и Flask (и почему)](#какая-разница-в-быстродействии-между-django-и-flask-и-почему)**
- **[Как в django работает система аутентификации](#как-в-django-работает-система-аутентификации)**

## [Веб-разработка](#веб-разработка)

- **[Что такое CGI. Плюсы, минусы](#что-такое-cgi-плюсы-минусы)**
...
- **[Что такое XSS. Примеры. Как защитить приложение](#что-такое-xss-примеры-как-защитить-приложение)**
- **[REST & SOAP](#rest-soap)**
  ...
  - [Как бы вы решили какой из REST или SOAP веб сервисов использовать](#как-бы-вы-решили-какой-из-rest-или-soap-веб-сервисов-использовать)
- **[Какие способы для мониторинга веб-приложений в production вы использовали или знаете](#какие-способы-для-мониторинга-веб-приложений-в-production-вы-использовали-или-знаете)**
- **[Как устроен протокол HTTP](#как-устроен-протокол-http)**
...
- **[Какие принципы программирования вы знаете](#какие-принципы-программирования-вы-знаете)**
  - [KISS](#kiss)
  - [DRY](#dry)
  - [YAGNI](#yagni)
  - [SLAP](#slap)
  - [SOLID принципы](#solid-принципы)
- **[Что такое code cohesion & code coupling](#что-такое-code-cohesion-code-coupling)**
- **[Какие шаблоны проектирования вы знаете](#какие-шаблоны-проектирования-вы-знаете)**
...
- **[Что такое lru cache](#что-такое-lru-cache)**
- **[Что такое MQ](#что-такое-mq)**
- **[Какие готовые реализации MQ вы знаете](#какие-готовые-реализации-mq-вы-знаете)**
- **[Что такое RPC](#что-такое-rpc)**
- **[Что такое gPRC](#что-такое-gprc)**

## [Алгоритмы и структуры данных](#алгоритмы-и-структуры-данных)

- **[Что такое рекурсия. Какие минусы, плюсы](#что-такое-рекурсия-какие-минусы-плюсы)**
- **[Что такое хвостовая рекурсия](#что-такое-хвостовая-рекурсия)**
- **[Как можно оптимизировать хвостовую рекурсию в Python](#как-можно-оптимизировать-хвостовую-рекурсию-в-python)**
- **[О-большое при оценке сложности](#о-большое-при-оценке-сложности)**
- **[Простой поиск](#простой-поиск)**
- **[Бинарный поиск](#бинарный-поиск)**
- **[Рекурсивные алгоритмы](#рекурсивные-алгоритмы)**
- **[Быстрая сортировка](#быстрая-сортировка)**
- **[Граф](#граф)**
- **[Очередь и стек](#очередь-и-стек)**
- **[Дерево](#дерево)**
- **[Поиск в ширину](#поиск-в-ширину)**
- **[Алгоритм Дейкстры](#алгоритм-дейкстры)**
- **[Жадные алгоритмы](#жадные-алгоритмы)**
- **[Как распознать NP-полную задачу](#как-распознать-np-полную-задачу)**
- **[Динамическое программирование](#динамическое-программирование)**
- **[Алгоритм k ближайших соседей](#алгоритм-k-ближайших-соседей)**
- **[Алгоритм Ричарда Фейнмана](#алгоритм-ричарда-фейнмана)**
...
- **[Что такое JWT (JSON Web Token)](#что-такое-jwt-json-web-token)**

## [SDLC](#sdlc)

- **[Agile/Scrum](#agilescrum)**
- **[Какая разница между CI и CD](#какая-разница-между-ci-и-cd)**
- **[Какая разница между Scrum и Kanban](#какая-разница-между-scrum-и-kanban)**
- **[Вопрос для тим-лидов: что Вы будете делать, если на проекте нет тестов и заказчик не хочет тратить на их разработку время и деньги](#вопрос-для-тим-лидов-что-вы-будете-делать-если-на-проекте-нет-тестов-и-заказчик-не-хочет-тратить-на-их-разработку-время-и-деньги)**
...
- **[Что такое транзакция. Какие у неё есть свойства](#что-такое-транзакция-какие-у-неё-есть-свойства)**
- **[Какие команды управления транзакциями вы знаете](#какие-команды-управления-транзакциями-вы-знаете)**
- **[Что такое уровни изолированности транзакций. Какие они бывают](#что-такое-уровни-изолированности-транзакций-какие-они-бывают)**
- **[Что такое вложенные транзакции](#что-такое-вложенные-транзакции)**
- **[Что такое курсор и зачем он нужен](#что-такое-курсор-и-зачем-он-нужен)**
- **[Какая разница между PostgreSQL и MySQL](#какая-разница-между-postgresql-и-mysql)**
- **[Что такое VACUUM в PostgreSQL](#что-такое-vacuum-в-postgresql)**
- **[Что такое EXPLAIN. Какая разница между ним и EXPLAIN ANALYZE](#что-такое-explain-какая-разница-между-ним-и-explain-analyze)**
- **[Какие виды Join'ов вы знаете, чем они отличаются друг от друга](#какие-виды-joinов-вы-знаете-чем-они-отличаются-друг-от-друга)**

## [Дизайн-интервью](#дизайн-интервью)

- **[План интервью](#план-интервью)**
- **[1. Сбор требований](#1-сбор-требований)**
- **[2. Эстимейты](#2-эстимейты)**
- **[3. API](#3-api)**
...
## [Django (расширенное)](#django-расширенное)

- **[Что такое QuerySet и в чём его ленивость](#что-такое-queryset-и-в-чём-его-ленивость)**
- **[Что такое select_related и prefetch_related](#что-такое-select-related-и-prefetch-related)**
- **[Что такое N+1 проблема и как её решить](#что-такое-n1-проблема-и-как-её-решить)**
- **[Что такое миграции в Django и как они работают](#что-такое-миграции-в-django-и-как-они-работают)**
- **[Как работает система прав (permissions) в Django](#как-работает-система-прав-permissions-в-django)**
- **[Что такое Django signals и когда их лучше не использовать](#что-такое-django-signals-и-когда-их-лучше-не-использовать)**
- **[Что такое django-admin и как его расширить](#что-такое-django-admin-и-как-его-расширить)**
- **[Что такое Context Processor в Django](#что-такое-context-processor-в-django)**
- **[Что такое Custom Manager в Django](#что-такое-custom-manager-в-django)**
- **[Как оптимизировать производительность Django приложения](#как-оптимизировать-производительность-django-приложения)**

## [DRF](#drf)

- **[Что такое ViewSet и чем он отличается от APIView](#что-такое-viewset-и-чем-он-отличается-от-apiview)**
- **[Что такое Router в DRF](#что-такое-router-в-drf)**
- **[Что такое Authentication и Permission в DRF](#что-такое-authentication-и-permission-в-drf)**
- **[Как работает валидация в DRF сериализаторах](#как-работает-валидация-в-drf-сериализаторах)**
- **[Что такое Throttling в DRF](#что-такое-throttling-в-drf)**
- **[Что такое Filtering, Searching, Ordering в DRF](#что-такое-filtering-searching-ordering-в-drf)**
- **[Что такое Pagination в DRF](#что-такое-pagination-в-drf)**
- **[Что такое Nested Serializer](#что-такое-nested-serializer)**
- **[Чем отличается `create` от `update` в сериализаторе](#чем-отличается-create-от-update-в-сериализаторе)**

## [Celery](#celery)

- **[Что такое Celery и для чего он используется](#что-такое-celery-и-для-чего-он-используется)**
- **[Как объявить и вызвать задачу Celery](#как-объявить-и-вызвать-задачу-celery)**
- **[Что такое Celery Beat](#что-такое-celery-beat)**
- **[Что такое retry в Celery задачах](#что-такое-retry-в-celery-задачах)**
- **[Что такое Celery Canvas (chain, group, chord)](#что-такое-celery-canvas-chain-group-chord)**
- **[Как мониторить Celery задачи](#как-мониторить-celery-задачи)**

## [RabbitMQ](#rabbitmq)

- **[Что такое RabbitMQ и как он работает](#что-такое-rabbitmq-и-как-он-работает)**
- **[Какие типы Exchange существуют в RabbitMQ](#какие-типы-exchange-существуют-в-rabbitmq)**
- **[Что такое acknowledgment (ack) в RabbitMQ](#что-такое-acknowledgment-ack-в-rabbitmq)**
- **[Что такое Dead Letter Exchange (DLX)](#что-такое-dead-letter-exchange-dlx)**
- **[Чем RabbitMQ отличается от Redis как брокер Celery](#чем-rabbitmq-отличается-от-redis-как-брокер-celery)**

## [PostgreSQL (расширенное)](#postgresql-расширенное)

- **[Что такое индекс и какие типы индексов есть в PostgreSQL](#что-такое-индекс-и-какие-типы-индексов-есть-в-postgresql)**
- **[Что такое EXPLAIN и EXPLAIN ANALYZE](#что-такое-explain-и-explain-analyze)**
- **[Что такое транзакция и ACID](#что-такое-транзакция-и-acid)**
- **[Какие уровни изоляции транзакций существуют](#какие-уровни-изоляции-транзакций-существуют)**
- **[Что такое VACUUM и почему он нужен](#что-такое-vacuum-и-почему-он-нужен)**
- **[Что такое репликация в PostgreSQL](#что-такое-репликация-в-postgresql)**
- **[Что такое партиционирование таблиц](#что-такое-партиционирование-таблиц)**

## [Redis](#redis)

- **[Что такое Redis и какие структуры данных он поддерживает](#что-такое-redis-и-какие-структуры-данных-он-поддерживает)**
- **[Как Redis используется для кеширования в Django](#как-redis-используется-для-кеширования-в-django)**
- **[Что такое TTL в Redis](#что-такое-ttl-в-redis)**
- **[Что такое Redis Pub/Sub](#что-такое-redis-pubsub)**
- **[Что такое Redis Sentinel и Redis Cluster](#что-такое-redis-sentinel-и-redis-cluster)**
- **[Как обеспечить атомарность нескольких команд в Redis](#как-обеспечить-атомарность-нескольких-команд-в-redis)**

## [Docker](#docker)

- **[Что такое Docker и зачем он нужен](#что-такое-docker-и-зачем-он-нужен)**
- **[Чем Image отличается от Container](#чем-image-отличается-от-container)**
- **[Из чего состоит Dockerfile](#из-чего-состоит-dockerfile)**
- **[Что такое Docker Compose](#что-такое-docker-compose)**
- **[Что такое Docker Volume](#что-такое-docker-volume)**
- **[Что такое Docker Network](#что-такое-docker-network)**
- **[Какие best practices при написании Dockerfile](#какие-best-practices-при-написании-dockerfile)**

## [Kubernetes](#kubernetes)

- **[Что такое Kubernetes и зачем он нужен](#что-такое-kubernetes-и-зачем-он-нужен)**
- **[Какова архитектура Kubernetes кластера](#какова-архитектура-kubernetes-кластера)**
...
- **[Что такое Liveness и Readiness Probes](#что-такое-liveness-и-readiness-probes)**

## [Вопросы работодателю](#вопросы-работодателю)

- **[Вопросы HR'у](#вопросы-hrу)**
- **[Вопросы для технического собеседования](#вопросы-для-технического-собеседования)**
...
# Django (расширенные вопросы)


# Django (расширенное)

## Что такое QuerySet и в чём его ленивость

QuerySet — это объект Django ORM, представляющий набор объектов из базы данных. QuerySet является **ленивым (lazy)**: запрос к БД не выполняется в момент создания QuerySet, а только тогда, когда данные реально нужны — при итерации, срезе, вызове `list()`, `len()`, `bool()` и т.д.

```python
# Этот код НЕ обращается к БД
...
QuerySet'ы также **кешируют результаты**: при повторном обращении к уже вычисленному QuerySet SQL-запрос не повторяется.

## Что такое select_related и prefetch_related

`select_related` и `prefetch_related` — оптимизации для уменьшения числа запросов к БД при работе с отношениями.

**`select_related`** — делает `JOIN` в одном SQL-запросе. Подходит для ForeignKey и OneToOneField (связи «многие к одному» или «один к одному»):
...
**`prefetch_related`** — делает отдельные запросы, а потом соединяет результаты в Python. Подходит для ManyToMany и обратных ForeignKey:

```python
# prefetch_related делает 2 запроса вместо N+1
...
N+1 — антипаттерн, когда вместо одного запроса к БД выполняется N+1 запросов: один для получения списка объектов и ещё по одному для каждого связанного объекта.

```python
# Плохо: 1 + N запросов
orders = Order.objects.all()
for order in orders:
    print(order.user.email)  # каждый раз SELECT к таблице users
...
Для обнаружения N+1 используют `django-debug-toolbar` или `django-silk`.

## Что такое миграции в Django и как они работают

Миграции — это способ синхронизировать изменения в моделях Django с реальной схемой БД. Каждая миграция — это Python-файл, описывающий изменение схемы.

Основные команды:
- `makemigrations` — создаёт новые файлы миграций на основе изменений в моделях
- `migrate` — применяет миграции к БД
- `showmigrations` — показывает список миграций и их статус
- `sqlmigrate <app> <migration>` — показывает SQL, который будет выполнен

Django хранит историю применённых миграций в таблице `django_migrations`.

## Как работает система прав (permissions) в Django

Django предоставляет встроенную систему прав на уровне модели. Для каждой модели автоматически создаются четыре права: `add`, `change`, `delete`, `view`.

Проверка прав:
```python
user.has_perm('app.change_article')  # проверка права
user.has_perms(['app.add_article', 'app.change_article'])  # несколько прав
```
...
Помимо стандартных прав можно создавать кастомные через `Meta.permissions` в модели.

## Что такое Django signals и когда их лучше не использовать

Сигналы позволяют одним компонентам приложения уведомлять другие о произошедших событиях. Сигналы выполняются **синхронно** в том же потоке.
...
**Когда НЕ стоит использовать сигналы:**
- Когда отправитель и получатель находятся в одном модуле — лучше вызвать напрямую
- Для тяжёлых операций (отправка email, внешние запросы) — лучше Celery
- Когда нужна явная, читаемая логика — сигналы создают неявные зависимости
- `bulk_update` и `bulk_create` сигналы не вызывают

## Что такое django-admin и как его расширить

`django-admin` — встроенная административная панель Django. Позволяет управлять данными через веб-интерфейс.
...
- `actions` — кастомные действия над выборкой
- `raw_id_fields` — для ForeignKey с большим числом записей

## Что такое Context Processor в Django

Context Processor — функция, которая принимает объект запроса и возвращает словарь переменных, автоматически добавляемых в контекст каждого шаблона.

```python
def user_profile(request):
    return {'user_notifications': get_notifications(request.user)}
```

Подключается в `TEMPLATES[0]['OPTIONS']['context_processors']`.

Встроенные контекст-процессоры: `django.contrib.auth.context_processors.auth` (добавляет `user` и `perms`), `django.template.context_processors.request` (добавляет `request`).

## Что такое Custom Manager в Django

Менеджер (Manager) — интерфейс, через который Django предоставляет доступ к запросам к БД. По умолчанию Django создаёт менеджер `objects`. Можно создать кастомный:
...
# Использование
Article.published.all()  # только опубликованные
```

## Как оптимизировать производительность Django приложения

1. **ORM**: `select_related`, `prefetch_related`, `only()`, `defer()`, `values()`, `values_list()` для уменьшения числа запросов и объёма данных
2. **Индексы**: `db_index=True` на часто фильтруемых полях, составные индексы через `Meta.indexes`
3. **Кеширование**: `django.core.cache` с Redis/Memcached, `cache_page` декоратор, `@cached_property`
4. **Celery**: тяжёлые операции (отправка email, генерация отчётов) выносить в фоновые задачи
5. **Пагинация**: никогда не отдавать весь QuerySet без ограничения
6. **`django-debug-toolbar`**: профилировать запросы в dev-окружении
...
## Что такое ViewSet и чем он отличается от APIView

**`APIView`** — базовый класс для CBV в DRF. Позволяет определять методы `get`, `post`, `put`, `delete` вручную. Полный контроль, но много бойлерплейта.

**`ViewSet`** — объединяет логику нескольких связанных представлений. Методы называются `list`, `create`, `retrieve`, `update`, `destroy`.

**`ModelViewSet`** — комбинация ViewSet + миксинов, реализует CRUD автоматически:
...
## Что такое Router в DRF

Router автоматически генерирует URL-паттерны для ViewSet:
...
`DefaultRouter` дополнительно создаёт корневой API-эндпоинт и добавляет `.json` суффиксы.

## Что такое Authentication и Permission в DRF

**Authentication** — определяет, кто делает запрос. Встроенные классы:
- `SessionAuthentication` — для браузерных клиентов через куки
- `BasicAuthentication` — логин/пароль в заголовке
- `TokenAuthentication` — статический токен
- `JWTAuthentication` (через `djangorestframework-simplejwt`)

**Permission** — определяет, может ли пользователь выполнить действие:
...
## Как работает валидация в DRF сериализаторах

Валидация происходит при вызове `serializer.is_valid()`. Порядок:
1. Валидация поля (метод `validate_<fieldname>`)
2. Валидация на уровне объекта (метод `validate`)
3. Встроенные валидаторы полей
...
class ArticleSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if data['published_at'] < timezone.now():
            raise serializers.ValidationError("Нельзя публиковать в прошлом")
        return data
```

## Что такое Throttling в DRF

Throttling — ограничение частоты запросов (rate limiting). Встроенные классы:
- `AnonRateThrottle` — для анонимных пользователей (по IP)
- `UserRateThrottle` — для аутентифицированных (по user_id)
...
## Что такое Filtering, Searching, Ordering в DRF

`django-filter` и встроенные бэкенды позволяют фильтровать, искать и сортировать QuerySet через query-параметры:
...
class ArticleViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ['status', 'author']    # ?status=published
    search_fields = ['title', 'content']       # ?search=python
    ordering_fields = ['created_at', 'title']  # ?ordering=-created_at
```

## Что такое Pagination в DRF

Пагинация ограничивает количество объектов в ответе. Встроенные классы:
- `PageNumberPagination` — `?page=2&page_size=10`
- `LimitOffsetPagination` — `?limit=10&offset=20`
- `CursorPagination` — курсорная пагинация, эффективна для больших таблиц
...
## Что такое Nested Serializer

Nested Serializer позволяет включать связанные объекты непосредственно в ответ (вместо просто id):
...
Для записи вложенных объектов нужно переопределить `create` и `update`.

## Чем отличается `create` от `update` в сериализаторе

`create(validated_data)` вызывается при `serializer.save()` если не передан `instance`. Обычно создаёт новый объект:
...
`update(instance, validated_data)` вызывается когда `instance` передан в сериализатор:
...
Celery — распределённая очередь задач для Python. Используется для выполнения задач **асинхронно** (не блокируя HTTP-запрос) и **по расписанию**.

Типичные сценарии:
- Отправка email или SMS после регистрации
- Генерация PDF/Excel отчётов
- Интеграция с внешними API
- Очистка старых данных по расписанию
- Ресайз изображений после загрузки

Компоненты:
- **Producer** — Django-приложение, которое ставит задачу в очередь
- **Broker** — хранит очередь (RabbitMQ, Redis)
- **Worker** — процесс, который берёт задачи из очереди и выполняет их
- **Backend** — хранит результаты выполнения задач (Redis, Django DB)

## Как объявить и вызвать задачу Celery
...
def send_welcome_email(user_id):
    ...
    send_mail('Добро пожаловать!', '...', settings.FROM_EMAIL, [user.email])
    return f"Email sent to {user.email}"

# views.py — асинхронный вызов
send_welcome_email.delay(user.id)

# или с параметрами выполнения
send_welcome_email.apply_async(
    args=[user.id],
    countdown=60,      # запустить через 60 секунд
    expires=300,       # отменить если не запустилась за 5 минут
)
```

## Что такое Celery Beat

Celery Beat — планировщик задач (аналог cron). Запускает задачи по расписанию.
...
Beat-процесс нужно запускать отдельно: `celery -A proj beat -l info`

## Что такое retry в Celery задачах

Задачу можно перезапустить при ошибке через `self.retry()`:

```python
@shared_task(bind=True, max_retries=3)
def send_sms(self, phone, text):
    try:
        sms_api.send(phone, text)
    except SMSAPIException as exc:
        raise self.retry(exc=exc, countdown=2 ** self.request.retries)  # экспоненциальная задержка
```

`bind=True` даёт доступ к `self` (объекту задачи). `max_retries` ограничивает число попыток.

## Что такое Celery Canvas (chain, group, chord)

Canvas — примитивы для построения сложных workflows из задач.

**`chain`** — задачи выполняются последовательно, результат каждой передаётся следующей:
```python
(download_file.s(url) | process_file.s() | notify_user.s()).delay()
```

**`group`** — задачи выполняются параллельно:
```python
group(process_image.s(img_id) for img_id in image_ids).delay()
```

**`chord`** — group + callback: все параллельные задачи завершились → вызывается коллбэк:
```python
chord(
    group(resize.s(img_id) for img_id in ids),
    notify_complete.s()
).delay()
```

## Как мониторить Celery задачи

- **Flower** — веб-интерфейс для мониторинга: `celery -A proj flower`
- **Celery events**: `celery -A proj events`
- **Sentry** — отлавливает исключения в задачах
- **Prometheus + Grafana** через `celery-exporter`
...
RabbitMQ — брокер сообщений, реализующий протокол AMQP (Advanced Message Queuing Protocol). Позволяет приложениям общаться через сообщения асинхронно.

Основные компоненты:
- **Producer** — отправляет сообщения
- **Exchange** — принимает сообщения и маршрутизирует их в очереди
- **Queue** — буфер, хранящий сообщения
- **Consumer** — получает сообщения из очереди и обрабатывает их
- **Binding** — правило связи exchange с очередью

## Какие типы Exchange существуют в RabbitMQ

- **Direct** — сообщение направляется в очередь, у которой routing key точно совпадает с ключом сообщения
- **Fanout** — сообщение рассылается во **все** привязанные очереди (игнорирует routing key). Используется для broadcast
- **Topic** — маршрутизация по паттерну routing key: `*` — одно слово, `#` — ноль или более слов. Например, `logs.*.error` или `logs.#`
- **Headers** — маршрутизация по заголовкам сообщения (редко используется)

## Что такое acknowledgment (ack) в RabbitMQ

ACK (acknowledgment) — подтверждение обработки сообщения. Пока consumer не отправил ack, RabbitMQ считает сообщение "в обработке". Если worker упал — сообщение возвращается в очередь.

Режимы:
- **auto-ack** — сообщение считается обработанным сразу после доставки. Быстро, но сообщение теряется при крэше воркера
- **manual ack** — consumer явно подтверждает после обработки. Надёжно

```python
channel.basic_ack(delivery_tag=method.delivery_tag)   # успех
channel.basic_nack(delivery_tag=method.delivery_tag, requeue=True)  # вернуть в очередь
```

## Что такое Dead Letter Exchange (DLX)

DLX — специальный exchange, куда попадают сообщения, которые:
- Были отклонены (nack/reject) без requeue
- Истёк TTL сообщения
- Очередь переполнена

Позволяет реализовать отложенные задачи и анализировать "битые" сообщения.

## Чем RabbitMQ отличается от Redis как брокер Celery

| Характеристика | RabbitMQ | Redis |
|---|---|---|
| Протокол | AMQP | проприетарный |
| Надёжность | Высокая, персистентность из коробки | Зависит от настроек AOF/RDB |
| Маршрутизация | Гибкая (exchange типы) | Простые очереди (lists) |
| Сложность | Выше | Ниже |
| Дополнительные возможности | DLX, priority queues | Кеш, pub/sub, структуры данных |

Для продакшена с высокими требованиями к надёжности и сложной маршрутизацией — RabbitMQ. Для простых сценариев или если Redis уже используется — Redis.
...
# PostgreSQL (расширенное)

## Что такое индекс и какие типы индексов есть в PostgreSQL

Индекс — структура данных, ускоряющая поиск по таблице. Без индекса — Sequential Scan (полный перебор строк).

Основные типы:
- **B-tree** (по умолчанию) — подходит для сравнений (`=`, `<`, `>`, `BETWEEN`, `LIKE 'abc%'`)
- **Hash** — только для проверки равенства (`=`), быстрее B-tree для этого случая
- **GiST** — обобщённое дерево поиска, для геоданных (PostGIS), полнотекстового поиска
- **GIN** — для составных значений: массивы, jsonb, tsvector (полнотекстовый поиск)
- **BRIN** — блочный индекс, эффективен для очень больших таблиц с натуральной сортировкой (временные ряды)
...
На что смотреть:
- `Seq Scan` — последовательный scan, плохо для больших таблиц
- `Index Scan` / `Index Only Scan` — использует индекс
- `cost=X..Y` — ориентировочная стоимость (в условных единицах)
...
- **Isolation (Изолированность)** — транзакции не видят промежуточные состояния друг друга
...
| Read Committed (по умолчанию) | нет | да | да |
| Repeatable Read | нет | нет | нет* |
| Serializable | нет | нет | нет |

*PostgreSQL всегда предотвращает Dirty Read даже на Read Uncommitted

## Что такое VACUUM и почему он нужен

PostgreSQL использует MVCC (Multi-Version Concurrency Control): при UPDATE старая строка не удаляется физически, а помечается как "мёртвая" (dead tuple). Это позволяет параллельным транзакциям видеть корректные снимки данных.

`VACUUM` физически удаляет мёртвые туплы и возвращает место. `VACUUM ANALYZE` дополнительно обновляет статистику для планировщика запросов.

`AUTOVACUUM` — фоновый процесс, запускающийся автоматически. В большинстве случаев достаточно настроить его параметры.

## Что такое репликация в PostgreSQL

Репликация — копирование данных на дополнительные серверы.

**Streaming Replication** (стандартный подход):
- Primary (master) — принимает запись
- Standby (replica) — только чтение
- Репликация через WAL (Write-Ahead Log)
- **Синхронная** — primary ждёт подтверждения от реплики (надёжно, медленнее)
- **Асинхронная** — primary не ждёт (быстрее, возможна потеря данных)

В Django для чтения с реплики используют `DATABASE_ROUTERS`.

## Что такое партиционирование таблиц

Партиционирование — разбивка большой таблицы на несколько физически отдельных частей (партиций). Запросы с фильтром по ключу партиционирования обращаются только к нужным партициям.
...
Типы: RANGE (диапазон), LIST (список значений), HASH.
...
## Что такое Redis и какие структуры данных он поддерживает

Redis — in-memory хранилище данных (key-value store). Хранит данные в оперативной памяти, поэтому работает очень быстро (~100K операций/сек).

Основные структуры данных:
- **String** — строки, числа, бинарные данные. Используется для кеширования, счётчиков
- **List** — двусвязный список. Используется как очередь (LPUSH/RPOP)
- **Hash** — словарь полей. Хранение объектов
- **Set** — множество уникальных элементов. Теги, уникальные посетители
- **Sorted Set (ZSet)** — множество с очками (score). Рейтинги, очереди с приоритетами
- **Bitmap** — операции с битами. Трекинг активности
- **Stream** — лог событий (аналог Kafka, с версии 5.0)

## Как Redis используется для кеширования в Django
...
def get_article(pk):
    ...
    if article is None:
        ...
        cache.set(cache_key, article, timeout=300)  # кеш на 5 минут
    return article
...
TTL (Time-To-Live) — время жизни ключа в секундах. После истечения ключ автоматически удаляется.

```bash
SET session:abc123 "user_data"
EXPIRE session:abc123 3600  # истекает через 1 час

TTL session:abc123  # сколько секунд осталось
PERSIST session:abc123  # убрать TTL (сделать постоянным)
```

Политики вытеснения при переполнении памяти (`maxmemory-policy`):
- `noeviction` — ошибка при записи
- `allkeys-lru` — вытеснять наименее давно используемые
- `volatile-lru` — LRU только среди ключей с TTL
- `allkeys-random` — случайные ключи

## Что такое Redis Pub/Sub

Pub/Sub — паттерн публикации/подписки. Publisher отправляет сообщения в канал, все Subscribers получают их.
...
# Publisher
r.publish('notifications', 'New order #1234')
...
**Ограничения**: сообщения не персистентны, если subscriber не слушает — сообщение теряется. Для надёжной очереди лучше использовать Streams или Celery.

## Что такое Redis Sentinel и Redis Cluster

**Redis Sentinel** — система высокой доступности. Мониторит master, при его падении автоматически делает один из slave новым master (failover). Подходит для одного шарда с репликацией.

**Redis Cluster** — горизонтальное масштабирование. Данные автоматически шардируются по 16384 слотам между несколькими master-нодами. Каждый master имеет свои slaves. Обеспечивает масштабирование как объёма данных, так и пропускной способности.

## Как обеспечить атомарность нескольких команд в Redis
...
EXPIRE views:article:42 86400
EXEC
```
Команды буферизуются и выполняются атомарно. Если между MULTI и EXEC придёт другая команда от другого клиента — она выполнится после EXEC.

**Lua-скрипты** (более мощный вариант):
```bash
EVAL "local n = redis.call('INCR', KEYS[1]); redis.call('EXPIRE', KEYS[1], ARGV[1]); return n" 1 views:42 86400
```
Скрипт выполняется атомарно, поддерживает условную логику.
...
Docker — платформа для контейнеризации приложений. Контейнер — изолированный процесс с собственной файловой системой, сетью и набором зависимостей, упакованный в образ.

**Зачем**:
- **Воспроизводимость**: "работает на моей машине" устранено — контейнер работает одинаково везде
- **Изоляция**: зависимости приложения не конфликтуют с системными
- **Быстрое развёртывание**: запуск за секунды
- **Масштабируемость**: легко запустить N копий

## Чем Image отличается от Container

**Image (образ)** — неизменяемый шаблон: слоёная файловая система, инструкции окружения. Аналог класса.

**Container (контейнер)** — запущенный экземпляр образа с доступным для записи слоем. Аналог объекта. При удалении контейнера изменения в этом слое теряются (если не использовать volumes).
...
COPY requirements.txt .         # копируем только requirements сначала (кеширование слоёв)
...
EXPOSE 8000                     # документирует порт (не открывает его!)

CMD ["gunicorn", "myproject.wsgi:application", "--bind", "0.0.0.0:8000"]
```

**Важно**: каждая инструкция `RUN`, `COPY`, `ADD` создаёт новый слой. Слои кешируются — если файл не изменился, слой перестраивать не нужно.

## Что такое Docker Compose

Docker Compose — инструмент для запуска **многоконтейнерных** приложений. Описывает все сервисы, сети и тома в одном `docker-compose.yml`:
...
Volume — механизм для **персистентного хранения данных** вне контейнера. Данные в томе живут независимо от жизненного цикла контейнера.

Типы:
- **Named Volume** (`postgres_data:/var/lib/postgresql/data`) — управляется Docker, хранится в `/var/lib/docker/volumes/`
- **Bind Mount** (`./code:/app`) — монтирует директорию хоста в контейнер. Используется для dev-режима
- **tmpfs** — хранится только в памяти, не записывается на диск
...
Контейнеры в одной Docker-сети могут обращаться друг к другу по имени сервиса. Docker Compose автоматически создаёт bridge-сеть для всех сервисов.

Типы сетей:
- **bridge** (по умолчанию) — изолированная сеть на одном хосте
- **host** — контейнер использует сетевой стек хоста напрямую
- **none** — нет сети
- **overlay** — для Docker Swarm, соединяет контейнеры на разных хостах

## Какие best practices при написании Dockerfile

1. Использовать **минимальные базовые образы** (`alpine`, `slim`)
2. **Кешировать зависимости**: копировать `requirements.txt` и устанавливать пакеты до копирования кода
3. Использовать **`.dockerignore`** (аналог `.gitignore`)
4. Не запускать процессы под `root` — создать отдельного пользователя
5. **Multi-stage builds** для уменьшения размера образа:
```dockerfile
...
COPY . .
```
6. Один процесс на контейнер
7. ENV-переменные для конфигурации, не хардкодить секреты
...
Kubernetes (K8s) — система оркестрации контейнеров с открытым исходным кодом. Автоматизирует развёртывание, масштабирование и управление контейнеризированными приложениями.
...
- **Управление конфигурацией** (ConfigMap, Secret)
- **Self-healing**: заменяет нерабочие поды

## Какова архитектура Kubernetes кластера

**Control Plane** (управляющий уровень):
- **API Server** — точка входа для всех операций (kubectl, другие компоненты)
- **etcd** — распределённое key-value хранилище, хранит состояние кластера
- **Scheduler** — назначает поды на ноды
- **Controller Manager** — поддерживает желаемое состояние (ReplicaSet, Node controller и др.)

**Worker Nodes** (рабочие узлы):
- **kubelet** — агент на каждой ноде, управляет подами
- **kube-proxy** — сетевое проксирование, реализует Service
- **Container Runtime** — Docker, containerd, CRI-O

## Что такое Pod

Pod — минимальная единица деплоя в Kubernetes. Содержит один или несколько контейнеров, которые:
- Всегда запускаются на одной ноде
- Разделяют IP-адрес и порты
- Разделяют volumes

Как правило, один pod = один контейнер приложения. Несколько контейнеров в pod — паттерн sidecar (например, основное приложение + логагрегатор).
...
Deployment — контроллер, управляющий ReplicaSet и обеспечивающий желаемое количество реплик пода. Поддерживает rolling updates и rollback.
...
kubectl rollout undo deployment/django-app  # откат
```

## Что такое Service

Service — абстракция для доступа к набору подов. Поды создаются и уничтожаются динамически — Service обеспечивает стабильный endpoint.

Типы:
- **ClusterIP** (по умолчанию) — доступен только внутри кластера
- **NodePort** — открывает порт на каждой ноде (30000-32767)
- **LoadBalancer** — создаёт внешний балансировщик (облачные провайдеры)
...
Ingress — объект для управления внешним HTTP/HTTPS трафиком. Позволяет настроить маршрутизацию по hostname и path, SSL-терминацию.
...
Ingress Controller (nginx-ingress, traefik) — компонент, реализующий правила Ingress.

## Что такое ConfigMap и Secret

**ConfigMap** — хранит конфигурацию в виде key-value, доступна подам как переменные окружения или файлы:
```yaml
apiVersion: v1
kind: ConfigMap
metadata:
...
**Secret** — аналог ConfigMap, но для чувствительных данных. Значения хранятся в base64 (не зашифрованы по умолчанию, для шифрования нужен KMS или Sealed Secrets):
```yaml
apiVersion: v1
kind: Secret
metadata:
...
HPA автоматически изменяет число реплик Deployment в зависимости от нагрузки:
...
При CPU > 70% HPA увеличит число реплик (до maxReplicas). При снижении — уменьшит (до minReplicas).

## Что такое Liveness и Readiness Probes

**Liveness Probe** — проверяет, жив ли контейнер. Если нет — kubelet перезапускает его.

**Readiness Probe** — проверяет, готов ли контейнер принимать трафик. Если нет — под убирается из балансировки Service.
